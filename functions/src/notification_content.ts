...
/**
 * Deterministically gets the variant ID for a specific user and test
 * @param {string} userId The user ID
 * @param {string} testId The test ID
 * @return {string} The variant ID (e.g., 'control', 'variant_a')
 */
export function getVariantForUser(userId: string, testId: string): string {
  ...
  // Create a deterministic hash of userId + testId
  const hash = crypto.createHash("sha256").update(`${userId}-${testId}`).digest("hex");

  // Take first 8 chars (32 bits) and parse to int
  const hashInt = parseInt(hash.substring(0, 8), 16);

  // Modulo by number of variants to get index
...
/**
 * Gets the notification content for a specific user based on their A/B test group
 * @param {string} userId The user ID
 * @param {string} testId The test ID
 * @param {Record<string, string>} params Parameters for copy replacement
 * @return {Object} Object containing title, body, and the assigned variantId
 */
