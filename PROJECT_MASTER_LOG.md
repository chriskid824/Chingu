...
**最新完成 (2025-11-25):**
- ✅ **活動報名管理系統 (Event Registration System)**:
  - **核心邏輯**:
    - `DinnerEventService` 實作完整的報名/取消流程 (Transaction)。
    - 支援候補名單 (Waitlist) 自動遞補邏輯。
    - 實作防呆機制：重複報名檢查、時間衝突檢查、滿員檢查。
    - 實作 24 小時前取消截止限制。
  - **資料模型**:
    - 更新 `DinnerEventModel` 支援 `maxParticipants`, `waitingList`。
    - 新增 `EventRegistrationStatus` Enum。
    - `UserModel` 新增 `noShowCount`。
  - **UI 實作**:
    - **活動詳情頁 (`EventDetailScreen`)**: 動態顯示報名狀態、候補人數，並整合報名/取消按鈕。
    - **我的活動頁 (`MyEventsScreen`)**: 分頁顯示「即將參加」、「候補中」、「歷史紀錄」。
    - **報名對話框 (`EventRegistrationDialog`)**: 統一的確認/取消互動介面。
  - **測試**:
    - 建立 `dinner_event_service_test.dart` 覆蓋所有核心業務邏輯 (100% pass)。
