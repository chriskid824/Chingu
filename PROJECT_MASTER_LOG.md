...
- ✅ **活動提醒系統 (Event Reminders)**:
  - **資料模型**: 更新 `DinnerEventModel` 新增 `is24hReminderSent` 與 `is2hReminderSent` 欄位
  - **Cloud Functions**: 實作 `sendEventReminders` 定時任務 (每 15 分鐘)
  - **推播邏輯**: 自動發送活動前 24 小時與 2 小時的 FCM 通知給已報名用戶
