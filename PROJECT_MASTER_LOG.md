...
### 🆕 最新完成 (Current Session):
- ✅ **活動報名系統 (Tasks 173-182)**:
  - **資料模型**:
    - 新增 `EventStatus` Enum。
    - 更新 `DinnerEventModel` 支援等候清單 (`waitingList`) 和 報名截止時間 (`registrationDeadline`)。
  - **服務邏輯**:
    - 增強 `DinnerEventService`：
      - 實作 `joinWaitlist`, `leaveWaitlist`。
      - 實作取消報名時自動從候補名單遞補 (`leaveEvent` promotion logic)。
      - 實作報名截止與人數限制檢查。
  - **UI 實作**:
    - 更新 `EventDetailScreen`：
      - 動態顯示活動狀態 (已確認/已額滿/報名截止)。
      - 整合 `EventRegistrationDialog` 進行報名/取消/候補確認。
      - 支援取消報名與退出候補。
    - 實作 `EventHistoryScreen` (原 `events_list_screen.dart`)：
      - 連接 `getUserEvents` 顯示真實報名歷史。
      - 支援下拉刷新與分頁 (Upcoming/History)。
      - 優化 `EventCard` 支援動態狀態標籤與顏色。
  - **測試**:
    - 建立 `test/services/dinner_event_service_test.dart` 覆蓋核心邏輯。
...
#### 活動模組 (Events)
- [x] 活動列表 (EventHistoryScreen) ✅
- [x] 活動詳情 (EventDetailScreen) ✅
- [ ] 建立活動
- [ ] 預約確認
- [ ] 活動評價
