...
class DinnerEventModel {
  ...
  DinnerEventModel({
    ...
    this.status = DinnerEventStatus.pending,
  ...
  factory DinnerEventModel.fromMap(Map<String, dynamic> map, String id) {
    // 處理 status 字串轉 enum
    DinnerEventStatus parseStatus(String? statusStr) {
      if (statusStr == null) return DinnerEventStatus.pending;
      try {
        return DinnerEventStatus.values.firstWhere(
          (e) => e.name == statusStr,
          orElse: () => DinnerEventStatus.pending,
        );
      } catch (_) {
        return DinnerEventStatus.pending;
      }
    }
  ...
  String get statusText {
    switch (status) {
      case DinnerEventStatus.pending:
        return '等待配對';
      case DinnerEventStatus.confirmed:
        return '已確認';
      case DinnerEventStatus.completed:
        return '已完成';
      case DinnerEventStatus.cancelled:
        return '已取消';
    }
  }
