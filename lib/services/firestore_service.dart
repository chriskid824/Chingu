...
class FirestoreService {
  ...
  ///
  /// [history] 登入歷史資料
  Future<void> recordLoginHistory(LoginHistoryModel history) async {
    try {
  ...
  /// [uid] 用戶 ID
  /// [limit] 返回數量限制
  Future<List<LoginHistoryModel>> getLoginHistory(String uid,
      {int limit = 20}) async {
    try {
      final querySnapshot = await _usersCollection
          .doc(uid)
      ...
      return querySnapshot.docs
          .map((doc) => LoginHistoryModel.fromMap(
              doc.data(), doc.id))
          .toList();
    } catch (e) {
