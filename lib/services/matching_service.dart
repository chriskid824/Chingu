...
class MatchingService {
  ...
  Future<List<Map<String, dynamic>>> getMatches(UserModel currentUser, {int limit = 10}) async {
    try {
      ...
      // 2.5. 獲取封鎖的用戶 ID
      final reportBlockService = ReportBlockService(firestore: _firestore);
      final blockedIds = await reportBlockService.getBlockedUserIds(currentUser.uid);
  ...
  Future<bool> _checkMutualMatch(String userId, String targetUserId) async {
    try {
      ...
      if (query.docs.isNotEmpty) {
        // 配對成功！（由 recordSwipe 呼叫 _handleMatchSuccess 處理後續）
        return true;
      }
      return false;
    } catch (e) {
