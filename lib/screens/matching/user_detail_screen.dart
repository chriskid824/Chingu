...
class _UserDetailScreenState extends State<UserDetailScreen> {
  ...
  bool _isFavorite(UserModel? currentUser) {
    if (currentUser == null || widget.user == null) return false;
    return currentUser.favoriteUserIds.contains(widget.user!.uid);
  }
  ...
  Future<void> _toggleFavorite() async {
    ...
    if (currentUser == null || targetUser == null || _isProcessing) return;
    ...
    try {
      final isFav = _isFavorite(currentUser);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(isFav ? '已移除收藏' : '已加入收藏'),
            ...
            actions: [
              Consumer<AuthProvider>(
                builder: (context, authProvider, child) {
                  final isFav = _isFavorite(authProvider.userModel);
                  return IconButton(
                    icon: Container(
                      padding: const EdgeInsets.all(8),
                      decoration: BoxDecoration(
                      ...
                      child: _isProcessing
                        ...
                        : Icon(
                            ...
                            size: 24,
                            color: isFav ? theme.colorScheme.primary : theme.colorScheme.onSurface,
                          ),
                    ),
