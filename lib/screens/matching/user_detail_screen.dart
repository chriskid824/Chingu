...
class _UserDetailScreenState extends State<UserDetailScreen> {
  ...
  Future<void> _checkFavoriteStatus() async {
    ...
    try {
      final isFav = await _firestoreService.isFavorite(currentUser.uid, widget.user!.uid);
      if (mounted) {
        setState(() {
          _isFavorited = isFav;
          _isLoading = false;
  ...
  Future<void> _toggleFavorite() async {
    ...
    try {
      if (_isFavorited) {
        await _firestoreService.removeFavorite(currentUser.uid, widget.user!.uid);
      } else {
        await _firestoreService.addFavorite(currentUser.uid, widget.user!.uid);
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(_isFavorited ? '已加入收藏' : '已從收藏移除'),
  ...
  Widget build(BuildContext context) {
    ...
    // Default values if user is null (preserving original mock data for fallback)
                  ...
                  Wrap(
                    ...
                    children: user != null
                        ? user.interests.map((interest) =>
                            _buildInterestChip(interest, Icons.local_activity_rounded, theme.colorScheme.primary)).toList()
                        : [
