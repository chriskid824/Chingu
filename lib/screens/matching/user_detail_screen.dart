...
class UserDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (args is UserModel) {
      user = args;
    } else if (args is Map<String, dynamic> && args['user'] is UserModel) {
      user = args['user'] as UserModel;
    }

    // Use user data if available, otherwise fallback to hardcoded data
    final name = user?.name ?? '陳大明';
    final age = user?.age ?? 30;
    final job = user?.jobTitle ?? '軟體工程師';
    ...
    final gender = user?.gender == 'female' ? '女性' : '男性';
    // Mock data for things not in UserModel or if user is null
    final budgetRange = 'NT\$ 500-800';
    final matchType = user?.gender == 'female' ? '異性配對' : '同性配對'; // Simple logic for display
    final isMockData = user == null;
      ...
      body: CustomScrollView(
        slivers: [
          SliverAppBar(
            ...
            actions: [
              if (user != null) // Only show report if we have a valid user
                Padding(
                    ...
                    child: Center(
                      child: user?.avatarUrl != null && user!.avatarUrl!.isNotEmpty
                        ? CachedNetworkImage(
                            ...
                            fit: BoxFit.cover,
                            placeholder: (context, url) => const Icon(Icons.person, size: 140, color: Colors.white),
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: interests.map((interest) {
                      return _buildInterestChip(interest, Icons.check_circle_outline, theme.colorScheme.primary);
                    }).toList(),
                  ),
                  
                  const SizedBox(height: 32),
                  
                  // 操作按鈕 - Always show action buttons
                  Row(
                    children: [
                      Expanded(
