...
class NotificationSettingsScreen extends StatelessWidget {
  ...
  void _updateSetting(BuildContext context, String key, bool value) {
    ...
    // Optimistically update or just call provider which updates user model
    authProvider.updateUserData({'notificationSettings': currentSettings});
  }

  bool _getSetting(AuthProvider provider, String key, {bool defaultValue = true}) {
    return provider.userModel?.notificationSettings[key] ?? defaultValue;
  }
      ...
      body: Consumer<AuthProvider>(
        builder: (context, authProvider, child) {
          if (authProvider.isLoading) {
             // Optional: Show loading indicator overlay or just rely on the switch animation
          }
          ...
          return ListView(
            children: [
              ...
              SwitchListTile(
                ...
                value: _getSetting(authProvider, 'pushEnabled'),
              ...
              SwitchListTile(
                ...
                value: _getSetting(authProvider, 'newMatch'),
                onChanged: (v) => _updateSetting(context, 'newMatch', v),
                activeColor: theme.colorScheme.primary,
              ),
              SwitchListTile(
                title: const Text('配對成功'),
                subtitle: Text('當配對成功時通知', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.6))),
                value: _getSetting(authProvider, 'matchSuccess'),
                onChanged: (v) => _updateSetting(context, 'matchSuccess', v),
                activeColor: theme.colorScheme.primary,
              ),
              const Divider(),
              ...
              SwitchListTile(
                ...
                value: _getSetting(authProvider, 'newMessage'),
                onChanged: (v) => _updateSetting(context, 'newMessage', v),
                activeColor: theme.colorScheme.primary,
              ),
              ListTile(
              ...
              SwitchListTile(
                ...
                value: _getSetting(authProvider, 'dinnerReminder'),
                onChanged: (v) => _updateSetting(context, 'dinnerReminder', v),
                activeColor: theme.colorScheme.primary,
              ),
              SwitchListTile(
                title: const Text('預約變更'),
                subtitle: Text('當預約有變更時通知', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.6))),
                value: _getSetting(authProvider, 'dinnerChange'),
                onChanged: (v) => _updateSetting(context, 'dinnerChange', v),
                activeColor: theme.colorScheme.primary,
              ),
              const Divider(),
              ...
              SwitchListTile(
                ...
                value: _getSetting(authProvider, 'promotions', defaultValue: false),
              ...
              SwitchListTile(
                ...
                value: _getSetting(authProvider, 'newsletter', defaultValue: false),
