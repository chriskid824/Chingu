...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> _recordLogin(String uid) async {
    try {
      // 1. Get Device Info
      String deviceInfoStr = 'Unknown Device';
      ...
      try {
        if (Platform.isAndroid) {
          ...
          deviceInfoStr = '${iosInfo.name} ${iosInfo.systemName} ${iosInfo.systemVersion}';
        }
      } catch (e) {
        debugPrint('Error getting device info: $e');
      }

      // 2. Get IP and Location (Best effort)
      String location = 'Unknown Location';
      String? ipAddress;

      try {
        // Using ipwho.is (free HTTPS)
        final response = await http.get(Uri.parse('https://ipwho.is/')).timeout(const Duration(seconds: 3));
