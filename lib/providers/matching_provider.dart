...
class MatchingProvider with ChangeNotifier {
  ...
  Future<Map<String, dynamic>?> swipe(String userId, String targetUserId, bool isLike) async {
    try {
      ...
      await AnalyticsService().logSwipe(targetUserId: targetUserId, isLike: isLike);

      if (result['isMatch'] == true) {
        final partner = result['partner'] as UserModel;
        await AnalyticsService().logMatch(partnerId: partner.uid);
